...
function AuthGate({onAuthenticated}:{onAuthenticated:()=>void}){
  ...
  return <main className="auth-gate">
    ...
    <p className="auth-subtitle">One secure sign-in for customers and admins. Your approved workspace opens automatically.</p>
...
export default function Home(){const {state,busy,toast,error,act,retry}=useGoocart();if(error?.code==="AUTH_REQUIRED")return <AuthGate onAuthenticated={()=>void retry()}/>;if(error)return <main className="fatal"><Brand/><h1>Goocart couldn’t start</h1><p>{error.message}</p><button onClick={()=>void retry()}>Try again</button></main>;if(!state)return <main className="loading"><Brand/><div className="loader"/><p>Starting Goocart...</p></main>;const content=state.actor.role==="CUSTOMER"?<Customer state={state} act={act} busy={busy}/>:state.actor.role.includes("ADMIN")?<Admin state={state} act={act} busy={busy}/>:<main className="fatal"><Brand/><h1>Workspace unavailable</h1><p>Your account role is not configured for this app. Vendor and delivery partner accounts use the Goocart mobile apps.</p><button onClick={()=>void logout()}>Sign out</button></main>;return <div>{busy&&<div className="syncing">Syncing…</div>}{toast&&<div className="toast">{toast}</div>}{content}</div>}
